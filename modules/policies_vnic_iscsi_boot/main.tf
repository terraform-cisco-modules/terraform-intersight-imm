...
#
# Intersight Ethernet (vNIC) iSCSI Boot Policy
...
resource "intersight_vnic_iscsi_boot_policy" "vnic_iscsi_boot" {
  ...
  dynamic "initiator_ip_pool" {
    ...
    content {
      moid = initiator_ip_pool.value.moid
    }
  }
  ...
  dynamic "primary_target_policy" {
    ...
    content {
      moid = primary_target_policy.value.moid
    }
  }
  dynamic "secondary_target_policy" {
    for_each = var.secondary_target_policy
    content {
      moid = secondary_target_policy.value.moid
    }
  }
  dynamic "tags" {
    for_each = var.tags
